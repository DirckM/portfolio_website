...
When the weekly generator invokes this skill it hands you the entry in the prompt
and works in a git worktree where the queue file does not exist. In that case do
not go looking for the queue and do not tick anything off. The wrapper script
owns the queue: it tops it up with researched topics when it runs low, and ticks
the entry off in the main checkout only after the render gate passes.
