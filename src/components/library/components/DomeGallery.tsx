...
export default function DomeGallery({
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      (el.style as unknown as { zIndex: string }).zIndex = '0';
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      animatingOverlay.style.cssText = `position:absolute;left:${overlayRelative.left}px;top:${overlayRelative.top}px;width:${overlayRelative.width}px;height:${overlayRelative.height}px;z-index:9999;border-radius:${openedImageBorderRadius};overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,.35);transition:all ${enlargeTransitionMs}ms ease-out;pointer-events:none;filter:${grayscale ? 'grayscale(1)' : 'none'};`;
      ...
      requestAnimationFrame(() => {
        animatingOverlay.style.left = originalPosRelative.left + 'px';
        animatingOverlay.style.top = originalPosRelative.top + 'px';
        animatingOverlay.style.width = originalPosRelative.width + 'px';
        animatingOverlay.style.height = originalPosRelative.height + 'px';
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      velocityX = 0;
      velocityY = 0;
      if (e.pointerType === 'touch') lockScroll();
    };
    ...
    const onPointerUp = (e: PointerEvent) => {
      ...
      if (wasMoved) {
        startInertia(velocityX * 60, velocityY * 60);
        lastDragEndAt.current = performance.now();
      }
      startPosRef.current = null;
      movedRef.current = false;
      if (e.pointerType === 'touch') unlockScroll();
    };
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <div ref={frameRef} className='viewer-frame w-full h-full' />
          </div>

          <div
            ref={scrimRef}
            className='scrim absolute inset-0 z-10 opacity-0 pointer-events-none transition-opacity duration-300'
            style={{
              background: `radial-gradient(ellipse at center, transparent 30%, ${overlayBlurColor} 90%)`,
