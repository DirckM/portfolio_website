...
// pnpm add @react-three/rapier meshline

let BallCollider: React.ComponentType<any> = () => null;
let CuboidCollider: React.ComponentType<any> = () => null;
let Physics: React.ComponentType<any> = ({ children }: any) => children;
let RigidBody: React.ComponentType<any> = ({ children }: any) => (
  <group>{children}</group>
);
let useRopeJoint: (...args: any[]) => void = () => {};
let useSphericalJoint: (...args: any[]) => void = () => {};
...
try {
  ...
  useRopeJoint = rapier.useRopeJoint;
  useSphericalJoint = rapier.useSphericalJoint;
} catch {
  // rapier not available
}
...
try {
  ...
  if (MeshLineGeometry && MeshLineMaterial) {
    extend({ MeshLineGeometry, MeshLineMaterial });
  }
} catch {
  // meshline not available
}

const CARD_GLB = '/assets/3d/card.glb';
const LANYARD_TEXTURE = '/assets/textures/lanyard.png';
...
export default function LanyardImpl({
  position = [0, 0, 30],
  gravity = [0, -40, 0],
  fov = 20,
  transparent = true,
