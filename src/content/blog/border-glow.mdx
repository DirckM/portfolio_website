...
If the border only has to turn rather than follow anything, that angle can come from a clock instead of a pointer, and the whole component becomes one keyframe. The [zero JavaScript version](/blog/gradient-border-css) does exactly that with a registered `@property` angle. Take this one when the glow has to react to the reader, take that one when it does not.
